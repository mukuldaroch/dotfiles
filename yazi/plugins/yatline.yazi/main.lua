...
--- Returns currently playing song from mpv, or empty string if none.
--- @return string song
function Yatline.string.get:song()
    local cmd = "echo '{ \"command\": [\"get_property\", \"media-title\"] }' | socat - /tmp/mpvsocket 2>&1"
    ...
    if result == "" or result:match("[Rr]efused") then
        return " "
    end

    -- try to extract the song title from JSON manually
    local song = result:match('"data"%s*:%s*"(.-)"')
    if not song or song == "null" then
        return " "
    end

    -- optionally add music icon
    return "🎵 " .. song
end
                ...
                if component.custom then
                    if component.name ~= nil and component.name ~= "" and #component.name ~= 0 then
                        section_components[#section_components + 1] = {
                            component_group.create(component.name, in_section),
                            component_group.has_separator,
                        }
                    end
                else
                    ...
                    if getter then
                        ...
                        if output ~= nil and output ~= "" then
                            section_components[#section_components + 1] = {
                                component_group.create(output, in_section),
                                component_group.has_separator,
                            }
                        end
                    end
