...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ]; then
        echo "hyprshade_filter=\"$choice\"" >~/.config/daroch/settings/hyprshade.sh
        if [ "$choice" == "off" ]; then
            hyprshade off
    ...
    if [ -f ~/.config/daroch/settings/hyprshade.sh ]; then
        source ~/.config/daroch/settings/hyprshade.sh
    fi
